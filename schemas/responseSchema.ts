...
export const RESPONSE_SCHEMA: Schema = {
  ...
  properties: {
    ...
    player_assertions: {
      ...
      nullable: true,
      items: { type: Type.STRING },
      description: "PLAYER-STATED CANON. 0-2 short declarative clauses capturing facts the PLAYER stated THIS TURN about their OWN character — history, training, gear, possessions and quantities remaining, skills, habits, what they are or are not. This is mostly ORDINARY SELF-DESCRIPTION offered in conversation, not just corrections; record it whenever it appears. Examples: 'growing up in the Orphanage in Ostmere' → 'Raised in an orphanage in Ostmere.'; 'security and circumvention of it is a strong skill set I possess' → 'Skilled at security systems and circumventing them.'; 'four vials of ink is what I started the day with and I have half a vial remaining' → 'Began the day with four vials of ink; roughly half a vial remains.'; 'the armor keeps me warm even my face' → 'Armor maintains body temperature over the whole body when deployed.' Record the fact, not the phrasing. Scope: only what the player actually stated, and only about their own character — not NPC thoughts, faction plans, or world events."
    },
